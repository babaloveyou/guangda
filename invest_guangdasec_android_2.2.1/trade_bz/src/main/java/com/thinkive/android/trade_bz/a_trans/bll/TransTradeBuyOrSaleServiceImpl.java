 ...
 * @date 2015/12/31
 */
public class TransTradeBuyOrSaleServiceImpl extends BasicServiceImpl {
    ...
    public TransTradeBuyOrSaleServiceImpl(TransTradeBuyOrSaleFragment fragment) {
        mFragment = fragment;
     ...
     * 可交易证券查询
     */
    public void getHoldList(String code) {
        HashMap<String, String> paramMap = new HashMap<String, String>();
        paramMap.put("stock_code",code);
        new Trans301703(paramMap, new IRequestAction() {
            @Override
            public void onSuccess(Context context, Bundle bundle) {
                ArrayList<TransSubHqBean> dataList = bundle.getParcelableArrayList(Trans301703.BUNDLE_KEY_SUB_SELECT);
                for(TransSubHqBean data : dataList){
                    data.setOrderprice(TradeUtils.formatDouble2(data.getOrderprice()));
                }

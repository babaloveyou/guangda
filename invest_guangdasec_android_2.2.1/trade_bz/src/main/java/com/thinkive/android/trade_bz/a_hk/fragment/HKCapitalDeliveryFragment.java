...
import com.thinkive.android.trade_bz.a_hk.adapter.HKCapitalDeliveryAdapter;
import com.thinkive.android.trade_bz.a_hk.bean.HKCapitalDeliveryBean;
import com.thinkive.android.trade_bz.a_stock.bll.BasicServiceImpl;
import com.thinkive.android.trade_bz.a_stock.controll.AbsBaseController;
import com.thinkive.android.trade_bz.a_stock.fragment.AbsBaseFragment;
import com.thinkive.android.trade_bz.interfaces.IRequestAction;
import com.thinkive.android.trade_bz.request.HK301637;
...
/**
 * 创建者     舒旺
 * 创建时间   2016/5/28 19:47
 * 描述	    交割单查询
 * 更新者     $Author$
 * 更新时间   $Date$
 */
public class HKCapitalDeliveryFragment extends AbsBaseFragment implements PullToRefreshListView.OnRefreshListener {

    private HKCapitalDeliveryController mCapitalDeliveryController;
    private Context mContext;
    private DatePickerSelect mDateSelect;
    private String mBegin;
    private String mEnd;
    private HKCapitalDeliveryServiceImpl mHkCapitalDeliveryService;
    private ListView mRefreshableView;
    private HKCapitalDeliveryAdapter mHkCapitalDeliveryAdapter;
    ...
    private ProgressBar mFragmentAbListInfoProgressbar;
    ...
    protected void findViews(View view) {
        ...
        mFragmentAbListInfoProgressbar = (ProgressBar) view.findViewById(R.id.fragment_ab_list_info_progressbar);
    ...
    protected void setListeners() {
        registerListener(AbsBaseController.ON_CLICK, mLinSelectDataStart, mCapitalDeliveryController);
        registerListener(AbsBaseController.ON_CLICK, mLinSelectDataEnd, mCapitalDeliveryController);
        registerListener(AbsBaseController.ON_CLICK, mTvSelectData, mCapitalDeliveryController);
    }

    @Override
    protected void initData() {
        mDateSelect = new DatePickerSelect(mContext);
        mCapitalDeliveryController = new HKCapitalDeliveryController(this);
        mHkCapitalDeliveryService = new HKCapitalDeliveryServiceImpl(this);
        mHkCapitalDeliveryAdapter = new HKCapitalDeliveryAdapter(mContext);
    ...
    protected void initViews() {
        ...
        mHkCapitalDeliveryService.reuqestDeliveryServiceImpl(mBegin, mEnd);
    }
     ...
     * 请求数据成功后回调给主线程
     */
    public void setDeliveryData(ArrayList<HKCapitalDeliveryBean> deliveryData) {
        if (null == deliveryData || deliveryData.size() <= 0) {
            //没有数据
            ...
            mHkCapitalDeliveryAdapter.setListData(deliveryData);
            mRefreshableView.setAdapter(mHkCapitalDeliveryAdapter);
        }
        refreshComplete();
    }
...
class HKCapitalDeliveryController extends AbsBaseController implements View.OnClickListener {

    private HKCapitalDeliveryFragment mHKCapitalDeliveryFragment;

    public HKCapitalDeliveryController(HKCapitalDeliveryFragment hkCapitalDeliveryFragment) {
        this.mHKCapitalDeliveryFragment = hkCapitalDeliveryFragment;
    }
    ...
    public void onClick(View v) {
        ...
        if (clickId == R.id.lin_select_data_start) { //调用系统日期  开始时间
            mHKCapitalDeliveryFragment.onClickBeginDate();
        } else if (clickId == R.id.lin_select_data_end) {//调用系统日期  结束时间
            mHKCapitalDeliveryFragment.onClickEndDate();
        } else if (clickId == R.id.tv_select_data) {
            mHKCapitalDeliveryFragment.onClickSelectDate();
        }
    }
}

class HKCapitalDeliveryServiceImpl extends BasicServiceImpl {

    private HKCapitalDeliveryFragment mHKCapitalLiabilitiesFragment;

    public HKCapitalDeliveryServiceImpl(HKCapitalDeliveryFragment hkCapitalLiabilitiesFragment) {
    ...
    public void reuqestDeliveryServiceImpl(String begin, String end) {
        ...
        new HK301637(map, new IRequestAction() {
            @Override
            public void onSuccess(Context context, Bundle bundle) {
                ArrayList<HKCapitalDeliveryBean> dataList = bundle.getParcelableArrayList(HK301637.BUNDLE_KEY_HK_HISTORY_TRADE);
                mHKCapitalLiabilitiesFragment.setDeliveryData(dataList);
            }

            @Override
            public void onFailed(Context context, Bundle bundle) {
                ToastUtils.toast(context, bundle.getString(HK301637.ERROR_INFO));
            }
        }).request();
    }
}
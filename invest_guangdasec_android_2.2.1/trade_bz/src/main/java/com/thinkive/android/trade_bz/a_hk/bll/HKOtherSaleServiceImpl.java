 ...
 * @date 2015/6/5
 */
public class HKOtherSaleServiceImpl extends BasicServiceImpl {

    private HKOtherSaleFragment mFragment = null;
    private LoadingDialogUtil loadingDialogUtil;//请求数据状态框 工具类

    public HKOtherSaleServiceImpl(HKOtherSaleFragment fragment) {
        mFragment = fragment;
    ...
    public void requestLinkageData(final String stock_code, String entrustPrice) {
        HashMap<String, String> paramMap = new HashMap<String, String>();
        paramMap.put("entrust_bs", "1");// 0：买入，1：卖出
        paramMap.put("stock_code", stock_code);
        paramMap.put("single_flag", "1");
    ...
    public void requestBuyOrSell() {
        ...
        paramMap.put("single_flag","1"); //是否零股申报（0：否，1：是)
        paramMap.put("max_price_levels","0");//最大价格等级(1:表示竞价限价盘 0:表示增强限价盘)

...
public class TransHistoryTradeAdapter extends AbsBaseAdapter<TransSelectBean> {
    /**
     * 上下文
     */
    ...
    public TransHistoryTradeAdapter(Context context) {
        super(context, R.layout.item_trans_history_trade);
        mContext = context;
    ...
    protected void onFillComponentData(ViewHolder holder, TransSelectBean bean) {
        ...
        TextView entrustBs= holder.getComponentById(R.id.tv_trans_history_trade_bs);
        ...
        TextView date= holder.getComponentById(R.id.tv_trans_history_trade_date);
        date.setText(bean.getEntrust_date()+ "  "+bean.getEntrust_time());

        TextView status = holder.getComponentById(R.id.tv_trans_history_trade_status);
        status.setText(bean.getEntrust_state_name());

        TextView name= holder.getComponentById(R.id.tv_trans_history_trade_name);
        name.setText(bean.getStock_name());

        TextView code= holder.getComponentById(R.id.tv_trans_history_trade_code);
        code.setText(bean.getStock_code());

        TextView entrustPrice= holder.getComponentById(R.id.tv_trans_history_trade_entrust_price);
        entrustPrice.setText(bean.getEntrust_price());

        TextView entrustNum= holder.getComponentById(R.id.tv_trans_history_trade_entrust_num);
        entrustNum.setText(bean.getEntrust_amount());

        TextView tradePrice= holder.getComponentById(R.id.tv_trans_history_trade_trade_price);
        tradePrice.setText(bean.getBusiness_price());

        TextView tradeNum= holder.getComponentById(R.id.tv_trans_history_trade_trade_num);
        tradeNum.setText(bean.getBusiness_amount());

        TextView tradeStatus= holder.getComponentById(R.id.tv_trans_history_trade_trade_status);
        tradeStatus.setText(bean.getEntrust_state_name());

        TextView yueNum= holder.getComponentById(R.id.tv_trans_history_trade_yue_num);
        yueNum.setText("????");

        TextView outXiNUm= holder.getComponentById(R.id.tv_trans_history_trade_out_xi);
        outXiNUm.setText("????");

        TextView outAccount= holder.getComponentById(R.id.tv_trans_history_trade_out_account);
        outAccount.setText("????");

...
public class OptionTodayTradeAdapter extends AbsBaseAdapter<OptionTodayTradeBean> {
    ...
    public OptionTodayTradeAdapter(Context context) {
        super(context, R.layout.item_option_today_trade);
        mContext = context;
        mFontManager = FontManager.getInstance(mContext);
    }

    @Override
    protected void onFillComponentData(ViewHolder holder, OptionTodayTradeBean bean) {
        ...
        tvEntrustStateName.setText(bean.getReal_status_name());
        //todo：缺字段
        tvOptionTypeName.setText("缺字段");
        tvOptholdTypeName.setText("缺字段");
        tvOptEntrustPrice.setText("缺字段");
        tvEntrustAmount.setText("缺字段");

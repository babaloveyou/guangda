...
import com.thinkive.android.trade_bz.request.Fund302007;
...
public class FundSubscribeServicesImpl {
    private FundSubscribeFragment mFragment = null;
    private LoadingDialogUtil loadingDialogUtil;//请求数据状态框 工具类

    public FundSubscribeServicesImpl(FundSubscribeFragment fragment) {
        mFragment = fragment;
     ...
     * @param money
     */
    public void requestFundSubscribe(String code,String money,String company,String isFlag) {
        loadingDialogUtil.showLoadingDialog(0);
        HashMap<String, String> map = new HashMap<String, String>();
        map.put("fund_code",code);
        ...
        new Fund302007(map, new IRequestAction() {
            @Override
            public void onSuccess(Context context, Bundle bundle) {
                loadingDialogUtil.hideDialog();
                PublicUseBean data = (PublicUseBean)bundle.getSerializable(Fund302007.BUNDLE_KEY_SUBTRICBUTE);
                mFragment.getFundSubscribe(data);
            }
            @Override
            public void onFailed(Context context, Bundle bundle) {
                loadingDialogUtil.hideDialog();
                String errorString = bundle.getString(Fund302007.ERROR_INFO);
                String errorNum =  bundle.getString(Fund302007.ERROR_NO);
    ...
    public void onGetErrorNum(Context context,String errorNum,String errorString){
        if(errorNum.equals("-30200710")){ // 未开户
            mFragment.openFundAccountDialog("-30200710",errorString);
        }else if(errorNum.equals("-30200711")){ // 风险预警
            mFragment.openFundAccountDialog("-30200711",errorString);
        }else{

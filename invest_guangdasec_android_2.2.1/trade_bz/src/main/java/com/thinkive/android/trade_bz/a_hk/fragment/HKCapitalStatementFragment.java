...
import com.thinkive.android.trade_bz.a_hk.adapter.HKCapitalStatementAdapter;
...
import com.thinkive.android.trade_bz.request.HK301636;
...
/**
 * 创建者     舒旺
 * 创建时间   2016/5/28 19:46
 * 描述	    沪港通对帐单查询
 * 更新者     $Author$
 * 更新时间   $Date$
 */
public class HKCapitalStatementFragment extends AbsBaseFragment implements PullToRefreshListView.OnRefreshListener {
    ...
    private HKCapitalStatementController mHkCapitalStatementController;
    private HKCapitalStatementServiceImpl mHkCapitalStatementService;
    private ListView mRefreshableView;
    private HKCapitalStatementAdapter mHkCapitalStatementAdapter;
    ...
    protected void setListeners() {
        registerListener(AbsBaseController.ON_CLICK, mLinSelectDataStart, mHkCapitalStatementController);
        registerListener(AbsBaseController.ON_CLICK, mLinSelectDataEnd, mHkCapitalStatementController);
        registerListener(AbsBaseController.ON_CLICK, mTvSelectData, mHkCapitalStatementController);
    }

    @Override
    protected void initData() {
        mDateSelect = new DatePickerSelect(mContext);
        mHkCapitalStatementController = new HKCapitalStatementController(this);
        mHkCapitalStatementService = new HKCapitalStatementServiceImpl(this);
        mHkCapitalStatementAdapter = new HKCapitalStatementAdapter(mContext);
    ...
    protected void initViews() {
        ...
        mHkCapitalStatementService.reuqestExchangeServiceImpl(mBegin,mEnd);
    }
     ...
     * 请求数据成功后回调给主线程
     */
    public void setStatementData(ArrayList<HKCapitalStatementBean> hkCapitalStatementBean) {
        if (null == hkCapitalStatementBean || hkCapitalStatementBean.size() <= 0) {
            //没有数据
            ...
            mHkCapitalStatementAdapter.setListData(hkCapitalStatementBean);
            mRefreshableView.setAdapter(mHkCapitalStatementAdapter);
        }
        refreshComplete();
    }
...
class HKCapitalStatementController extends AbsBaseController implements View.OnClickListener {

    private HKCapitalStatementFragment mHKCapitalStatementFragment;

    public HKCapitalStatementController(HKCapitalStatementFragment hkCapitalStatementFragment) {
        this.mHKCapitalStatementFragment = hkCapitalStatementFragment;
    }
    ...
    public void onClick(View v) {
        ...
        if (clickId == R.id.lin_select_data_start) { //调用系统日期  开始时间
            mHKCapitalStatementFragment.onClickBeginDate();
        } else if (clickId == R.id.lin_select_data_end) {//调用系统日期  结束时间
            mHKCapitalStatementFragment.onClickEndDate();
        } else if (clickId == R.id.tv_select_data) {
            mHKCapitalStatementFragment.onClickSelectDate();
        }
    }
}

class HKCapitalStatementServiceImpl extends BasicServiceImpl {

    private HKCapitalStatementFragment mHKCapitalLiabilitiesFragment;

    public HKCapitalStatementServiceImpl(HKCapitalStatementFragment hkCapitalLiabilitiesFragment) {
    ...
    public void reuqestExchangeServiceImpl(String begin, String end) {
        HashMap<String, String> map = new HashMap<String, String>();
        map.put("begin_date", begin);
        map.put("end_date", end);
        new HK301636(map, new IRequestAction() {
            @Override
            public void onSuccess(Context context, Bundle bundle) {
                ArrayList<HKCapitalStatementBean> dataList = bundle.getParcelableArrayList(HK301636.BUNDLE_KEY_HK_HISTORY_TRADE);
                mHKCapitalLiabilitiesFragment.setStatementData(dataList);
            }

            @Override
            public void onFailed(Context context, Bundle bundle) {
                ToastUtils.toast(context, bundle.getString(HK301636.ERROR_INFO));
            }
        }).request();
    }
}
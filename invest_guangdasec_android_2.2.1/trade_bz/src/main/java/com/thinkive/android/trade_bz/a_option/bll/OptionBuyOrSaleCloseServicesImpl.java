...
public class OptionBuyOrSaleCloseServicesImpl {
    private OptionBuyOrSaleCloseFragment mFragment = null;
    private Context mContext;
    private LoadingDialogUtil loadingDialogUtil;//请求数据状态框 工具类

    public OptionBuyOrSaleCloseServicesImpl(OptionBuyOrSaleCloseFragment fragment) {
        mFragment = fragment;
    ...
    public void startLinkage(final HashMap<String,String> params) {
        ...
        sendMsgToHqForStockList(stockCode, new OptionBuyOrSaleOpenServicesImpl.IHqCallBackStock() {
            @Override
     ...
     * 这里的模块通信异步返回结果。
     */
    private void sendMsgToHqForStockList(String curEditStockCode, final OptionBuyOrSaleOpenServicesImpl.IHqCallBackStock iHqCallBackStock) {
        try {
     ...
     * @param params
     */
    public void requestOptionHold(HashMap<String,String> params){
        HashMap<String,String> paramMap = new HashMap<String,String>();
        paramMap.put("opthold_type",params.get("optholdType"));//期权持仓类别（'0'-权利方 '1'-义务方 '2'-备兑方） 卖平---权利方 买平---义务方 备兑---备兑方
        new Option305003(paramMap, new IRequestAction() {
            @Override
            public void onSuccess(Context context, Bundle bundle) {
                if(bundle != null){
                    ArrayList<OptionHoldStockBean> optionHoldStockBeans = bundle.getParcelableArrayList(Option305003.BUNDLE_KEY_OPTION_HOLD);
                    mFragment.onGetOptionHoldList(optionHoldStockBeans);
                }
            }

            @Override
            public void onFailed(Context context, Bundle bundle) {
                String errorNo=bundle.getString(Option305003.ERROR_NO);
                String errorInfo=bundle.getString(Option305003.ERROR_INFO);
                LogUtil.printLog("e", "Option305003出错,error_no:" + errorNo + "||error_info:" + errorInfo);

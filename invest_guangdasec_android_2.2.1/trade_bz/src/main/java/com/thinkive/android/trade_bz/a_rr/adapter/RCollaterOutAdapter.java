...
public class RCollaterOutAdapter extends AbsBaseAdapter<MyStoreStockBean> {
    ...
    protected void onFillComponentData(ViewHolder holder, final MyStoreStockBean bean) {
        TextView name = (TextView) holder.getComponentById(R.id.tv_rollater_out_name);
        name.setText(bean.getStock_name());

        TextView code = (TextView) holder.getComponentById(R.id.tv_rollater_out_code);
        code.setText(bean.getStock_code());

        TextView nowPrice = (TextView) holder.getComponentById(R.id.tv_rollater_out_now_price);
        nowPrice.setText(bean.getLast_price());

        TextView yk = (TextView) holder.getComponentById(R.id.tv_rollater_out_yk);
        yk.setText(bean.getFloat_yk());

        TextView ykPer = (TextView) holder.getComponentById(R.id.tv_rollater_out_yk_per);
        ykPer.setText(bean.getFloat_yk_per());

        TextView chenBenJia = (TextView) holder.getComponentById(R.id.tv_rollater_out_ben);
        chenBenJia.setText(bean.getCost_price());

        TextView hold = (TextView) holder.getComponentById(R.id.tv_rollater_out_hold);
        hold.setText(bean.getEnable_amount());

        TextView zheSuanLv = (TextView) holder.getComponentById(R.id.tv_rollater_out_per_price);
        zheSuanLv.setText(bean.getPledge_rate());

        TextView market = (TextView) holder.getComponentById(R.id.tv_rollater_out_market);
    ...
    public void onGetLinkData(RCollaterLinkBean bean){
        RCollaterOutDialog dialog = new RCollaterOutDialog(mSubContext, mServices);

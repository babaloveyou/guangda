...
import com.thinkive.android.trade_bz.request.Request300206;
...
public class OneKeyServicesImpl {
     ...
     * 请求一键归集
     */
    public void requestOneKey(String passWord,String moneyType,String fundAccount) {
        loadingDialogUtil.showLoadingDialog(0);
        HashMap<String, String> map = new HashMap<String, String>();
        map.put("fund_account",fundAccount);
        map.put("password",mEncryptManager.getRsaEncryptStr(passWord));
        map.put("money_type", moneyType);
        new Request300206(map, new IRequestAction() {
            @Override
            public void onSuccess(Context context, Bundle bundle) {
                loadingDialogUtil.hideDialog();
                OneKeyBean data = (OneKeyBean)bundle.getSerializable(Request300206.BUNDLE_KEY_ONE_KEY);
                mFragment.getOneKeyResult(data);
            }
            @Override
            public void onFailed(Context context, Bundle bundle) {
                loadingDialogUtil.hideDialog();
                ToastUtils.toast(context, bundle.getString(Request300206.ERROR_INFO));
            }
        }).request();
    }
}
